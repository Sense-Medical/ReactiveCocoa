...
extension SignalType where Value: Sequence, Error == NoError {
...
extension SignalProducerType where Value: Sequence, Error == NoError {
...
extension SignalProducerType {
	...
	public func prefix<P: SignalProducerType>(_ previous: P) -> SignalProducer<Value, Error> where P.Value == Value, P.Error == Error {
...
extension SignalType {
	...
	public static func merge<Seq: Sequence, S: SignalType>(_ signals: Seq) -> Signal<Value, Error> where S.Value == Value, S.Error == Error, Seq.Iterator.Element == S {
	...
	/// Merges the given signals into a single `Signal` that will emit all
	/// values from each of them, and complete when all of them have completed.
	public static func merge<S: SignalType>(_ signals: S...) -> Signal<Value, Error> where S.Value == Value, S.Error == Error {
...
extension SignalProducerType {
	...
	public static func merge<Seq: Sequence, S: SignalProducerType>(_ producers: Seq) -> SignalProducer<Value, Error> where S.Value == Value, S.Error == Error, Seq.Iterator.Element == S {
	...
	/// all values from each of them, and complete when all of them have
	/// completed.
	public static func merge<S: SignalProducerType>(_ producers: S...) -> SignalProducer<Value, Error> where S.Value == Value, S.Error == Error {
